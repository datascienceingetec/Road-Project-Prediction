...
export const api = {
    // ---------- Auth ----------
    authGoogle(code: string): Promise<{ user: any; session: any }> {
    ...
    getMe(): Promise<{ user: any; session: any }> {
    ...
    async changeStatusProyecto(
        codigo: string,
        status: "active" | "inactive"
    ): Promise<boolean> {
        await fetchAPI(`/proyectos/${codigo}`, {
            method: "PUT",
    ...
    // ---------- Charts ----------
    async getValorPresenteCausacion(
        faseId?: number,
        alcance?: string,
        presentYear: number = 2025
    ) {
        ...
        return fetchAPI(
            `/charts/valor-presente-causacion?${params.toString()}`
        );
    },

    async getCausacionPorKm(
        faseId?: number,
        alcance?: string,
        presentYear: number = 2025
    ) {
    ...
    async uploadUFGeometry(ufId: number, file: File) {
        ...
        const response = await fetch(
            `${API_BASE_URL}/unidades-funcionales/${ufId}/geometry`,
            {
                method: "POST",
                body: formData,
                headers: {
    ...
    async updateUFGeometry(
        ...
        return fetchAPI(`/unidades-funcionales/${ufId}/geometry`, {
            ...
            body: JSON.stringify({
                geometry,
                recalculate_length: recalculateLength,
            }),
        });
    },
    ...
    async uploadProjectGeometries(
        ...
        const url = `${API_BASE_URL}/proyectos/${codigo}/geometries${
    ...
    async exportProjectGeometries(
        codigo: string,
        format: "kml" | "shp" | "geojson"
    ) {
        const response = await fetch(
            `${API_BASE_URL}/proyectos/${codigo}/geometries/export/${format}`,
            {
                headers: {
                    "ngrok-skip-browser-warning": "true",
                },
