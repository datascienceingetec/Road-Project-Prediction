...
from app.adapters.model_adapter import PhaseModelManager, LegacyModelAdapter
...
class ModelService:
    ...
    def __init__(self):
        self.adapter = LegacyModelAdapter()
        self.manager = PhaseModelManager(adapter=self.adapter)
    ...
    def parse_training_summary(self, model_data: Dict[str, Any]) -> Dict[str, List[Dict[str, Any]]]:
        ...
        if model_data.get('summary'):
            print(f"Loading {len(model_data['summary'])} metric rows from summary")
